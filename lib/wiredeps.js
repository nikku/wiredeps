...
function Wiredeps(options) {
  ...
  function resolveSnapshot(pkg, cb) {
    ...
    info.comittish = branch;

    hasPackageDescriptor(info, function(err, exists) {
      if (exists) {
        verboseLog('resolving ' + version + ' via ' + colors.yellow('<' + branch + '>'));
  ...
  this.wire = function(cb) {
    ...
    if (hasShrinkwrapConfig(cwd)) {
      log('a local ' + shrinkwrapDescriptor + ' already exists, skipping');
      return cb();
    }

    if (branch) {
      log('trying to resolve dependencies from ' + colors.yellow('<' + branch + '>'));
